<script setup>
const props = defineProps(['title','btnTitle','btnColor'])
    ...
    <div v-if="showModal"  class="pd-overlay bg-black bg-opacity-10  w-full h-full fixed top-0 left-0 z-[60] overflow-x-hidden overflow-y-auto">
        <div class="  ease-out  opacity-100 transition-all duration-500 md:max-w-3xl md:w-full m-3 md:mx-auto mt-20">
            <div class="flex flex-col bg-white min-h-60 rounded-2xl py-6 px-5">
                ...
                <slot :close-modal="()=> showModal = false" />
                
                <!-- <div class="flex items-center justify-end pt-4 border-t border-gray-200 space-x-4">
                    <button type="button" class="py-2.5 px-5 text-xs bg-blue-50 text-blue-500 rounded-full cursor-pointer font-semibold text-center shadow-xs transition-all duration-500 hover:bg-blue-100 close-modal-button" data-pd-overlay="#medium-modal" data-modal-target="medium-modal">Cancel</button>
                    <button type="button" class="py-2.5 px-5 text-xs  bg-blue-500 text-white rounded-full cursor-pointer font-semibold text-center shadow-xs transition-all duration-500 hover:bg-blue-700 close-modal-button" data-pd-overlay="#medium-modal" data-modal-target="medium-modal">Okay, got it</button>
                </div> -->
            </div>
        </div>
    </div>
